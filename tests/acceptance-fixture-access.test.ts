...
test("acceptance fixture access permits only named canonical fixtures and locales", () => {
  ...
  assert.deepEqual(parseAcceptanceFixture("ashen-crown", "en"), { name: "ashen-crown", locale: "en" });
...
test("fixture URL remains a server endpoint and does not duplicate fixture content", () => {
  ...
  assert.equal(acceptanceFixturePath({ name: "ashen-crown", locale: "ja" }), "/__acceptance-fixtures/ashen-crown.ja.e2r.json");
