...
export function CreditsDialog({ locale, onClose }: { locale: Locale; onClose: () => void }) {
  return <>
    ...
    <aside className="detail credits-dialog" role="dialog" aria-modal="true" aria-labelledby="credits-title">
      <div className="detail-header"><h3 id="credits-title">{translate(locale, "credits")}</h3><button type="button" onClick={onClose}>{translate(locale, "close")}</button></div>
      <dl>
        <dt>{translate(locale, "application")}</dt><dd>LiaisonScape 0.1.0</dd>
        <dt>{translate(locale, "creator")}</dt><dd>sukoyaka-dopeness</dd>
        <dt>{translate(locale, "releaseDate")}</dt><dd>{translate(locale, "releaseDatePending")}</dd>
      </dl>
