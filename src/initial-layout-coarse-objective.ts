...
export function scoreCoarseInitialLayout(input: CoarseObjectiveInput): CoarseObjectiveMetrics {
  const entityMap = new Map(input.entities.map((entity) => [entity.id, entity]));
  ...
  for (let left = 0; left < edgeRelations.length; left += 1) {
    ...
    relationLabelCorridorPressure += Math.max(0, width(first.label ?? first.id) / 2 + 24 - midpointDistance(firstMid, firstSource, firstTarget)) / 100;
