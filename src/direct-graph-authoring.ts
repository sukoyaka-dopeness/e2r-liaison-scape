...
export type ViewBoxSize = { x?: number; y?: number; width: number; height: number };
...
export function draggedPointFromOrigin(startValue: GraphPoint, startPointer: GraphPoint, currentPointer: GraphPoint): GraphPoint {
  return { x: startValue.x + currentPointer.x - startPointer.x, y: startValue.y + currentPointer.y - startPointer.y };
}
...
export function graphPointFromPointer(
  ...
  const view = svgPointFromPointer(pointer, rect, viewBox);
  if (!view || !Number.isFinite(scale) || scale <= 0) return null;
  return {
    x: viewBox.width / 2 + (view.x - viewBox.width / 2 - pan.x) / scale,
    y: viewBox.height / 2 + (view.y - viewBox.height / 2 - pan.y) / scale,
  };
}

export function svgPointFromPointer(pointer: PointerPosition, rect: SvgRect, viewBox: ViewBoxSize): GraphPoint | null {
  const viewBoxX = viewBox.x ?? 0;
  const viewBoxY = viewBox.y ?? 0;
  if (![pointer.clientX, pointer.clientY, rect.left, rect.top, rect.width, rect.height, viewBoxX, viewBoxY, viewBox.width, viewBox.height].every(Number.isFinite)) return null;
  if (rect.width <= 0 || rect.height <= 0 || viewBox.width <= 0 || viewBox.height <= 0) return null;
  const renderScale = Math.min(rect.width / viewBox.width, rect.height / viewBox.height);
  const offsetX = rect.left + (rect.width - viewBox.width * renderScale) / 2;
  const offsetY = rect.top + (rect.height - viewBox.height * renderScale) / 2;
  return { x: viewBoxX + (pointer.clientX - offsetX) / renderScale, y: viewBoxY + (pointer.clientY - offsetY) / renderScale };
}
