...
import artifact from "../../product-owned-bundle-local-capacity1/result-summary.json";
...
type Row = { fixture: string; positions: Record<string, Point>; bundleLocalJoint: { selected: { spacingByKey: Record<string, number> } } };
...
if (!row) throw new Error(`Unknown bundle-local fixture: ${fixtureId}`);
...
const preview = { operationId: 1, generation: 1, snapshotIdentity: `product-owned-bundle-local-capacity1-${fixtureId}-${candidateId}`, candidateFingerprint: fingerprintPreviewPositions(row.positions), positions: row.positions };
const parallelBundleVariant = candidateId === "fixed16" ? "bundle-16" : undefined;
const parallelBundleSpacingByKey = candidateId === "bundle-local" ? row.bundleLocalJoint.selected.spacingByKey : undefined;
document.title = `${candidateId} / ${fixtureId}`;
createRoot(document.getElementById("root")!).render(<StrictMode><App diagnosticDataset={dataset} operationLocalPreview={preview} parallelBundleVariant={parallelBundleVariant} parallelBundleSpacingByKey={parallelBundleSpacingByKey} /></StrictMode>);
